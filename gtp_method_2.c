...
void Save_Request_data(UC * buffer)
{
    ...
    fwrite(buffer,1,strlen(buffer),fp_data);
    
    if(memcmp(buffer,"Frame",5)==0 && gotdata==1)
    {
        if(fp_data!=NULL)
        {
            fclose(fp_data);
            ...
            while ((line_size = getline(&line, &len, fptr)) != -1)
            {
                //~ printf("%s",line,strlen(line)); fflush(stdout);
                
                memset(temp,0x00,100);
                memcpy(temp , line , (strlen(line)-1));
                
                if(memcmp(temp , "TEID: " , 6)==0)
                {
                    strcpy(GTP.TEID , temp+6);
                }
                else if(memcmp(temp , "Sequence number: " , 17)==0)
                {
                    strcpy(GTP.seq_no , temp+17);
                }
                else if(memcmp(temp , "IMSI: " , 6)==0)
                {
                    strcpy(GTP.IMSI , temp+6);
                }
                else if(memcmp(temp , "GSN address : " , 14)==0 && gsn==0)
                {
                    strcpy(GTP.GSN_IP1 , temp+14);
                    gsn=1;
                }
                else if(memcmp(temp , "GSN address : " , 14)==0 && gsn==1)
                {
                    strcpy(GTP.GSN_IP2 , temp+14);
                    gsn=0;
                }
                else if(memcmp(temp , "Address digits: " , 16)==0)
                {
                    strcpy(GTP.MSISDN , temp+16);
                }
                else if(memcmp(temp , "Mobile Country Code (MCC): " , 27)==0)
                {
                    memset(GTP.CGI_MCC , 0x00 , sizeof(GTP.CGI_MCC));
                    
                    for(i=27,j=0 ; i<strlen(temp+27) ; i++,j++)
                    {
                        if(temp[i]==' ')    break;
                        
                        GTP.CGI_MCC[j] = temp[i];
                    }
                    
                    //~ strcpy(GTP.CGI_MCC , temp+27);
                }
                else if(memcmp(temp , "Mobile Network Code (MNC): " , 27)==0)
                {
                    memset(GTP.CGI_MNC , 0x00 , sizeof(GTP.CGI_MNC));
                    
                    strcpy(GTP.CGI_MNC , temp+27);
                }
                else if(memcmp(temp , "Cell LAC: " , 10)==0)
                {
                    memset(GTP.CGI_LAC , 0x00 , sizeof(GTP.CGI_LAC));
                    strcpy(GTP.CGI_LAC , temp+10);
                }
                else if(memcmp(temp , "Cell CI: " , 9)==0)
                {
                    strcpy(GTP.CGI_CI , temp+9);
                    final_data=1;
                }
                else if(memcmp(temp , "IMEI(SV): " , 10)==0)
                {
                    strcpy(GTP.IMEI , temp+10);
                }
            }
            ...
            if(final_data==1)
            {
                ...
                printf("\nTEID : %s"    ,   GTP.TEID); fflush(stdout);
                printf("\nseq_no : %s"  ,   GTP.seq_no); fflush(stdout);
                printf("\nIMSI : %s"    ,   GTP.IMSI); fflush(stdout);
                printf("\nGSN_IP1 : %s" ,   GTP.GSN_IP1); fflush(stdout);
                printf("\nGSN_IP2 : %s" ,   GTP.GSN_IP2); fflush(stdout);
                printf("\nGTP.CGI_MCC : %s"  ,   GTP.CGI_MCC); fflush(stdout);
                printf("\nGTP.CGI_MNC : %s"  ,   GTP.CGI_MNC); fflush(stdout);
                ...
                ht_insert(ht, "TEID"                ,   GTP.TEID);
                ht_insert(ht, "SEQUENCE_NUMBER"     ,   GTP.seq_no);
                ht_insert(ht, "IMSI"                ,   GTP.IMSI);
                ht_insert(ht, "GSN_IP1"             ,   GTP.GSN_IP1);
                ht_insert(ht, "GSN_IP2"             ,   GTP.GSN_IP2);
                ht_insert(ht, "MSISDN"              ,   GTP.MSISDN);
                ht_insert(ht, "Mobile Country Code" ,   GTP.CGI_MCC);
                ht_insert(ht, "Mobile Network Code" ,   GTP.CGI_MNC);
                ht_insert(ht, "Location Area Code"  ,   GTP.CGI_LAC);
                ht_insert(ht, "Cell ID"             ,   GTP.CGI_CI);
                ht_insert(ht, "IMEI"                ,   GTP.IMEI);
...
void Save_Response_data(UC * buffer)
{
    ...
    if(memcmp(buffer,"Frame",5)==0 && gotdata==2)
    {
        if(fp_data!=NULL)
        {
            fclose(fp_data);
            ...
            while ((line_size = getline(&line, &len, fptr)) != -1)
            {
                ...
                else if(memcmp(temp , "Cause: " , 7)==0)
                {
                    strcpy(GTP.Cause , temp+7);
                    
                    if(strcmp(GTP.Cause , "Request accepted (128)")==0)
                    {
                        final_data=1;
                    }
                }
                else if(memcmp(temp , "End user address (IETF/IPv4) : " , 31)==0)
                {
                    strcpy(GTP.End_user_addr , temp+31);
                }
            }
            ...
            if(final_data==1)
            {
                ...
                printf("\nTEID : %s"            ,   GTP.TEID); fflush(stdout);
                printf("\nseq_no : %s"          ,   GTP.seq_no); fflush(stdout);
                printf("\nCause : %s"           ,   GTP.Cause); fflush(stdout);
                printf("\nEnd_user_addr : %s"   ,   GTP.End_user_addr); fflush(stdout);
                
                ht_insert(ht, "TEID"                ,   GTP.TEID);
                ht_insert(ht, "SEQUENCE_NUMBER"     ,   GTP.seq_no);
                ht_insert(ht, "CAUSE"               ,   GTP.Cause);
                ht_insert(ht, "End user address"    ,   GTP.End_user_addr);
...
void Save_session_request_data(UC * buffer)
{
    ...
    if(memcmp(buffer,"Frame",5)==0 && gotdata==3)
    {
        if(fp_data!=NULL)
        {
            fclose(fp_data);
            ...
            while ((line_size = getline(&line, &len, fptr)) != -1)
            {
                ...
                if(memcmp(temp , "Tunnel Endpoint Identifier: " , 28)==0)
                {
                    strcpy(GTP.TEID , temp+28);
                }
                else if(memcmp(temp , "Sequence Number: " , 17)==0)
                {
                    strcpy(GTP.seq_no , temp+17);
                }
                else if(memcmp(temp , "International Mobile Subscriber Identity (IMSI) : " , 50)==0)
                {
                    strcpy(GTP.IMSI , temp+50);
                }
                else if(memcmp(temp , "MSISDN : " , 9)==0)
                {
                    strcpy(GTP.MSISDN , temp+9);
                ...
                else if(memcmp(temp , "Mobile Equipment Identity (MEI) : " , 34)==0)
                {
                    strcpy(GTP.IMEI , temp+34);
                }
            }
            
            if(final_data==1 && (strlen(GTP.IMEI)>0))
            {
                final_data = 0;
...
void Save_session_response_data(UC * buffer)
{
    ...
    if(memcmp(buffer,"Frame",5)==0 && gotdata==4)
    {
        if(fp_data!=NULL)
        {
            fclose(fp_data);
            ...
            while ((line_size = getline(&line, &len, fptr)) != -1)
            {
                ...
                else if(memcmp(temp , "Cause: " , 7)==0)
                {
                    ...
                    if(strcmp(GTP.Cause , "Request accepted (16)")==0)
                    {
                        final_data=1;
                    }
                }
                else if(memcmp(temp , "PDN Address and Prefix(IPv4): " , 30)==0)
                {
                    strcpy(GTP.PDNaddr , temp+30);
                    final_data_1=1;
                }
            }
            
            if(final_data==1 && final_data_1==1)
            {
                final_data = 0;
                final_data_1 = 0;
                ...
                printf("\nseq_no : %s"  ,   GTP.seq_no); fflush(stdout);
                printf("\nCause : %s"   ,   GTP.Cause); fflush(stdout);
                printf("\nPDNaddr : %s"   ,   GTP.PDNaddr); fflush(stdout);
                
                ht_insert(ht, "TEID"                ,   GTP.TEID);
                ht_insert(ht, "SEQUENCE_NUMBER"     ,   GTP.seq_no);
                ht_insert(ht, "CAUSE"               ,   GTP.Cause);
                ht_insert(ht, "PDNaddr"             ,   GTP.PDNaddr);
...
void Save_data_Into_Mysql_GTP()
{
	...
	print_search(ht, "TEID" 		            , TEID);
	print_search(ht, "SEQUENCE_NUMBER" 		    , Seq_No);
	print_search(ht, "IMSI" 		            , IMSI);
	print_search(ht, "GSN_IP1" 		            , GSN_IP1);
	print_search(ht, "GSN_IP2" 		            , GSN_IP2);
    print_search(ht, "Mobile Country Code"      , CGI_MCC);
    print_search(ht, "Mobile Network Code"      , CGI_MNC);
    ...
    print_search(ht, "CAUSE"                    , Cause);
    print_search(ht, "End user address"         , End_user_addr);
    print_search(ht, "PDNaddr"                  , PDNaddr);
    
	//~ printf("\nMsg_type in Save_data_Into_Mysql_GTP() : %s\n",Msg_type); fflush(stdout);
    
	if(strcmp(Msg_type , "0x10")==0)
	{
        sprintf((char*)query , (const char*)"INSERT INTO request (MsgType , TEID , seq_no , IMSI , GSN_IP1 , GSN_IP2 , CGI_MCC , CGI_MNC , CGI_LAC , CGI_Cell_ID , MSISDN , IMEI) VALUES ('%s' , '%s' , '%s' , '%s' , '%s' , '%s' , '%s' , '%s' , '%s' , '%s' , '%s' , '%s')",
                    Msg_type,
                    TEID,
                    Seq_No,
                    IMSI,
	...
	else if(strcmp(Msg_type , "0x11")==0)
	{
        sprintf((char*)query , (const char*)"INSERT INTO response (MsgType , TEID , seq_no , Cause , End_user_addr) VALUES ('%s' , '%s' , '%s' , '%s' , '%s')",
                    Msg_type,
                    TEID,
                    Seq_No,
                    Cause,
	...
	{
        sprintf((char*)query , (const char*)"INSERT INTO session_request (MsgType , TEID , seq_no , IMSI , CGI_MCC , CGI_MNC , MSISDN , IMEI) VALUES ('%s' , '%s' , '%s' , '%s' , '%s' , '%s' , '%s' , '%s')",
                    Msg_type,
                    TEID,
                    Seq_No,
                    IMSI,
	...
	{
        sprintf((char*)query , (const char*)"INSERT INTO session_response (MsgType , TEID , seq_no , Cause , PDNaddr) VALUES ('%s' , '%s' , '%s' , '%s' , '%s')",
                Msg_type,
                TEID,
                Seq_No,
                Cause,
...
/****
 * CREATE TABLE request ( id INT NOT NULL PRIMARY KEY AUTO_INCREMENT, Time DATETIME DEFAULT CURRENT_TIMESTAMP, MsgType varchar(64) NOT NULL default '' , TEID varchar(32) NOT NULL default '' , seq_no varchar(64) NOT NULL default '' , IMSI varchar(64) NOT NULL default '' , GSN_IP1 varchar(64) default '' , GSN_IP2 varchar(15) NOT NULL default '' , CGI_MCC varchar(50) default NULL , CGI_MNC varchar(32) default NULL , CGI_LAC varchar(32) default NULL , CGI_Cell_ID varchar(32) default NULL , MSISDN varchar(32) default NULL , IMEI varchar(32) default NULL)
 * CREATE TABLE session_request ( id INT NOT NULL PRIMARY KEY AUTO_INCREMENT, Time DATETIME DEFAULT CURRENT_TIMESTAMP, MsgType varchar(64) NOT NULL default '' , TEID varchar(32) NOT NULL default '' , seq_no varchar(64) NOT NULL default '' , IMSI varchar(64) NOT NULL default '' , CGI_MCC varchar(50) default NULL , CGI_MNC varchar(32) default NULL, MSISDN varchar(64) NOT NULL default '' , IMEI varchar(32) default NULL)
 * CREATE TABLE response (id INT NOT NULL PRIMARY KEY AUTO_INCREMENT, Time DATETIME DEFAULT CURRENT_TIMESTAMP,MsgType varchar(64) NOT NULL default '',TEID varchar(32) NOT NULL default '',seq_no varchar(64) NOT NULL default '',Cause varchar(64) NOT NULL default '',End_user_addr varchar(64) NOT NULL default '')
 * CREATE TABLE session_response (id INT NOT NULL PRIMARY KEY AUTO_INCREMENT, Time DATETIME DEFAULT CURRENT_TIMESTAMP,MsgType varchar(64) NOT NULL default '',TEID varchar(32) NOT NULL default '',seq_no varchar(64) NOT NULL default '',Cause varchar(64) NOT NULL default '' , PDNaddr varchar(64) NOT NULL default '')
 */ 
void delete_create_table_GTP(MYSQL *con)
{
    ...
    //MsgType , TEID , seq_no , IMSI , GSN_IP1 , GSN_IP2 , CGI_MCC , CGI_MNC , CGI_LAC , CGI_Cell_ID , MSISDN , IMEI
    ...
    //MsgType , TEID , seq_no , IMSI,  CGI_MCC , CGI_MNC , MSISDN , IMEI
    ...
    //MsgType , TEID , seq_no, Cause, End_user_addr
    
	if (mysql_query(con, "CREATE TABLE response (id INT NOT NULL PRIMARY KEY AUTO_INCREMENT, Time DATETIME DEFAULT CURRENT_TIMESTAMP,MsgType varchar(64) NOT NULL default '',TEID varchar(32) NOT NULL default '',seq_no varchar(64) NOT NULL default '',Cause varchar(64) NOT NULL default '',End_user_addr varchar(64) NOT NULL default '')"))
	{
		finish_with_error(con);
	}
	...
	if (mysql_query(con, "CREATE TABLE session_response (id INT NOT NULL PRIMARY KEY AUTO_INCREMENT, Time DATETIME DEFAULT CURRENT_TIMESTAMP,MsgType varchar(64) NOT NULL default '',TEID varchar(32) NOT NULL default '',seq_no varchar(64) NOT NULL default '',Cause varchar(64) NOT NULL default '' , PDNaddr varchar(64) NOT NULL default '')"))
	{
		finish_with_error(con);
	}
}
...
int main()
{
    ...
    UC command      [1024]      =   {0};
    UC filename     [20]        =   "GTP.txt"; 
    UC buffer       [1024] 	    =    {0};
    UC msg_type     [50]        =   {0};
    
    FILE * fp=NULL;
    
    // sudo tshark -x -i enp175s0f0 -f "udp port 2123" -V -c 1 | sed 's/^[ \t]*//;s/[ \t]*$//' | awk '/GPRS Tunneling Protocol/{y=1;next}y'
     
    system("killall -9 tshark");
    
    sprintf((char*)command , (const char*)"rm %s",filename);
    system(command);
    
    memset(command , 0x00 , sizeof(command));
    
    //~ strcpy((char*)command , (const char*)"tshark -x -i enp175s0f0 -f \"udp port 2123\" -V | sed 's/^[ \t]*//;s/[ \t]*$//' | awk '/GPRS Tunneling Protocol/{y=1;next}y'");
    strcpy((char*)command , (const char*)"tshark -i enp175s0f0 -f \"udp port 2123\" -V | sed 's/^[ \t]*//;s/[ \t]*$//' | awk '/GPRS Tunneling Protocol/{y=1;next}y'");
    ...
    while(1)
    {
        if(fgets(buffer, sizeof(char)*500, fp) == NULL)
        {
            pclose(fp);
        }
        else
        {
            ...
            if(memcmp(buffer , "Message Type: Create PDP context request (0x10)",47)==0)
            {
                ...
                memcpy(msg_type,buffer+42,4);
                printf("\nmsg_type : %s",msg_type); fflush(stdout);
                ht_insert(ht, "MESSAGE_TYPE", msg_type);
            }
            else if(memcmp(buffer , "Message Type: Create PDP context response (0x11)",48)==0)
            {
                ...
                memset(msg_type,0x00,sizeof(msg_type));
                strncpy(msg_type,buffer+43,4);
                printf("\nmsg_type : %s",msg_type); fflush(stdout);
                ht_insert(ht, "MESSAGE_TYPE", msg_type);
            }
            else if(memcmp(buffer , "Message Type: Create Session Request (32)",41)==0)
            {
                ...
                memset(msg_type,0x00,sizeof(msg_type));
                strncpy(msg_type,buffer+38,2);
                printf("\nmsg_type : %s",msg_type); fflush(stdout);
                ht_insert(ht, "MESSAGE_TYPE", msg_type);
            }
            else if(memcmp(buffer , "Message Type: Create Session Response (33)",42)==0)
            {
